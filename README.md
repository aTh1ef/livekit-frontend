...
Frontend for the Gemini Live voice agent at https://github.com/aTh1ef/livekit-voice-agent

Forked from [livekit-examples/agent-starter-react](https://github.com/livekit-examples/agent-starter-react) (MIT).
...
Fill in `LIVEKIT_URL`, `LIVEKIT_API_KEY`, `LIVEKIT_API_SECRET` from the **same** LiveKit project the
agent uses, and set `AGENT_NAME=gemini-demo` to match `agent_name` in the agent's `src/agent.py`.
...
Source files are pinned to LF via `.gitattributes`, and `.prettierrc` sets `"endOfLine": "auto"`.
Without both, `pnpm build` fails with hundreds of `Delete ␍  prettier/prettier` errors.
